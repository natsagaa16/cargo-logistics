...
app.post('/api/containers-new', authenticate, checkRole(['system_admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      ...
      [container_code, name || null, direction_id, container_type_id,
       road_info_id, registration_date, departure_date, arrival_date, description || null, req.user.id]
    );
...
app.patch('/api/containers-new/:id/start-distribution', authenticate, checkRole(['system_admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    // ЗАСВАР: Эхлээд хэдэн ачаа байгааг тоолох
    ...
    // ЗАСВАР: Зөвхөн 'distributed' биш бүх ачааг шинэчлэх
    ...
    `, [req.user.id, id]);
    // Өөрчлөгдсөн batch-ууд болон тэдний ачааг авах
    ...
    `, [id]);
    // Batch бүрт зөвхөн нэг түүх үүсгэх
...
async function calculateStorageFee(cargo) {
  ...
  case 'per_piece':
    storageFeeAmount = storageDays * pricePerDay;
    break;
  case 'per_kg':
  ...
  storageFeeAmount *= (parseFloat(cargo.total_pieces) || 1);
...
app.post('/api/cargo-new', authenticate, checkRole(['system_admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    const [[container]] = await connection.query(`
  SELECT c.*, ct.price_per_kg, ct.price_per_cbm, ct.type_name, pc.currency_code
  FROM containers_new c
  JOIN container_types ct ON c.container_type_id = ct.id
  LEFT JOIN payment_currencies pc ON ct.currency_id = pc.id
...
`, [container_id]);
    ...
    if (cargo_type === 'weight') {
      unitPrice = parseFloat(weight_kg) * parseFloat(container.price_per_kg);
    } else if (cargo_type === 'volume') {
      volume_cbm = (parseFloat(length_cm) * parseFloat(width_cm) * parseFloat(height_cm)) / 1000000;
...
app.patch('/api/cargo-new/:id/status', authenticate, checkRole(['system_admin', 'staff']), async (req, res) => {
  ...
  try {
    ...
    // ===== ШИНЭ КОД ЭХЛЭЛ =====
    // Distributed төлөвөөс буцаахыг хориглох
    ...
    await connection.query(
      ...
      // Төлөвийн түүх хадгалах - багц бүрт зөвхөн нэг удаа
      await connection.query(
        `INSERT INTO cargo_status_history (cargo_id, batch_number, old_status, new_status, changed_by, notes)
        VALUES (?, ?, ?, ?, ?, ?)`,
        [cargo.id, cargo.batch_number, cargo.status, status, req.user.id, null]
      );
    // ХЭР "distributed" төлөвт шилжсэн БА өмнө нь distributed биш байсан бол
    if (status === 'distributed' && cargo.status !== 'distributed') {
      // Төлбөрийн хэлбэр шаардлагатай
      if (!payment_method) {
        await connection.rollback();
        return res.status(400).json({
          success: false,
      ...
      // Багцын нийт үнийг тооцох (давхардахгүй)
      ...
      // Payment record үүсгэх - ЗӨВХӨН НЭГ УДАА багц бүрт
      await connection.query(`
...
// ==================== ТӨЛБӨР АВЧ DISTRIBUTED ТӨЛӨВТ ШИЛЖҮҮЛЭХ ====================
...
// ==================== ТАРААГДСАН ТӨЛӨВ БУЦААХ (SYSTEM ADMIN) ====================
app.patch('/api/cargo-new/:id/reverse-distribution', authenticate, checkRole(['system_admin']), async (req, res) => {
  ...
  try {
    ...
    // Зөвхөн distributed төлөвийг буцаах боломжтой
    if (cargo.status !== 'distributed') {
      await connection.rollback();
      return res.status(400).json({
        success: false,
    ...
    // Төлбөрийн бүртгэлийг устгах
    const [deleteResult] = await connection.query(
      'DELETE FROM payment_records WHERE cargo_batch_number = ?',
      [cargo.batch_number]
    );
   
    // Төлөв буцаах - pending_distribution руу
    await connection.query(
      'UPDATE cargo_new SET status = ?, updated_by = ?, updated_at = NOW() WHERE batch_number = ?',
...
// ==================== PAYMENT STATS - ШИНЭЧИЛСЭН ====================
app.get('/api/payment/stats', authenticate, async (req, res) => {
  try {
    const { start_date, end_date, currency_code } = req.query;
    ...
    if (currency_code && currency_code !== 'all') {
      if (dateFilter) {
        dateFilter += ' AND currency_code = ?';
      } else {
        dateFilter = 'WHERE currency_code = ?';
      }
      params.push(currency_code);
    }
   
    // Нийт тараагдаагүй ачаа (багцаар)
    ...
    // Өнөөдрийн тараагдсан ачаа (багцаар)
...
app.get('/api/payment/records', authenticate, async (req, res) => {
  try {
    const { start_date, end_date, payment_type, payment_method, container_id, currency_code } = req.query;
    ...
    if (currency_code && currency_code !== 'all') {
      query += ' AND pr.currency_code = ?';
      params.push(currency_code);
    }
   
    let filter = '';
    if (start_date && end_date) {
      filter += ' AND pr.payment_date BETWEEN ? AND ?';
    }
    if (payment_type && payment_type !== 'all') {
      filter += ' AND pr.payment_type = ?';
    }
    if (payment_method && payment_method !== 'all') {
      filter += ' AND pr.payment_method = ?';
    }
    if (container_id && container_id !== 'all') {
      filter += ' AND pr.container_id = ?';
    }
    if (currency_code && currency_code !== 'all') {
      filter += ' AND pr.currency_code = ?';
    }

    const countQuery = `SELECT COUNT(*) as total FROM payment_records pr WHERE 1=1 ${filter}`;
    const countParams = [...params.slice(0, params.length - 2)]; // LIMIT OFFSET-ээс өмнөх params
    const [[{ total }]] = await dbPool.query(countQuery, countParams);
