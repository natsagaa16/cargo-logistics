...
const PaymentManagement = () => {
  ...
  const fetchContainers = async () => {
    try {
      ...
      const response = await axiosInstance.get('/api/containers-new');
  ...
  try {
    ...
    console.log('✅ Distributed cargos response:', response.data);  // DEBUG: Response-г шалгах
    if (response.data.success) {
      const cargosWithFees = response.data.data.map(cargo => {
        console.log('Cargo data:', cargo);  // DEBUG: Тухайн cargo-г шалгах
        return {
          ...cargo,
          storage_fee: cargo.storage_fee || cargo.storage_fee_amount || 0,  // backend-ээс storage_fee эсвэл storage_fee_amount-г авч, null бол 0
          total_amount: (cargo.price || 0) + (cargo.storage_fee || cargo.storage_fee_amount || 0)
        };
      });
      setDistributedCargos(cargosWithFees);
    }
  } catch (error) {
  ...
  try {
    ...
    console.log('✅ Revenue records response:', response.data);  // DEBUG: Response-г шалгах
    if (response.data.success) {
      const recordsWithFees = response.data.data.map(record => {
        console.log('Record data:', record);  // DEBUG: Тухайн record-г шалгах
        return {
          ...record,
          storage_fee: record.storage_fee || 0  // backend-ээс storage_fee-г авч, null бол 0
        };
      });
      setRevenueRecords(recordsWithFees);
    }
  } catch (error) {
  ...
  const formatCurrency = (amount, currency = 'KRW') => {
    ...
    const currency_symbol = symbols[currency] || currency;
    if (!amount) return `0 ${currency_symbol}`;
    return `${parseFloat(amount).toLocaleString()} ${currency_symbol}`;
  };
  ...
  const calculateTotals = (records, type) => {
    if (type === 'pending') {
      ...
      const totalCargo = records.reduce((sum, r) => sum + parseFloat(r.price || 0), 0);
      const totalStorage = records.reduce((sum, r) => sum + parseFloat(r.storage_fee || 0), 0);
      const grandTotal = totalCargo + totalStorage;
      return { totalCargo, totalStorage, grandTotal };
    }
            ...
            {activeTab === 'pending' && (
              <>
                <table className="data-table min-w-full">
                  ...
                  <tbody>
{pendingCargos.length === 0 ? <tr><td colSpan="8" className="text-center py-8 text-gray-600">Тараагдаагүй ачаа байхгүй</td></tr> : pendingCargos.map((cargo) => (
                        <tr key={cargo.id}>
            ...
            {activeTab === 'distributed' && (
              <>
                <table className="data-table min-w-full">
                  ...
                  <tbody>
                    {distributedCargos.length === 0 ? <tr><td colSpan="9" className="text-center py-8 text-gray-600">Тараагдсан ачаа байхгүй</td></tr> : distributedCargos.map((cargo) => (
                        <tr key={cargo.id}>
                          ...
                          <td className="whitespace-nowrap text-orange-600 font-semibold">{formatCurrency(cargo.storage_fee, cargo.price_currency_code)}</td>
                          <td className="whitespace-nowrap font-bold text-green-600">{formatCurrency(cargo.total_amount, cargo.price_currency_code)}</td>
                  ...
                  {distributedCargos.length > 0 && (
                    <tfoot className="bg-gray-50 font-bold">
                      <tr>
                        ...
                        <td className="text-green-600 text-lg">{formatCurrency(calculateTotals(distributedCargos, 'distributed').totalCargo)}</td>
                        <td className="text-orange-600">{formatCurrency(calculateTotals(distributedCargos, 'distributed').totalStorage)}</td>
                        <td className="text-blue-600 text-lg">{formatCurrency(calculateTotals(distributedCargos, 'distributed').grandTotal)}</td>
                        <td colSpan="2"></td>
                      </tr>
                    </tfoot>
                  )}
            ...
            {activeTab === 'revenue' && (
              <>
                <table className="data-table min-w-full">
                  ...
                  <tbody>
                    {revenueRecords.length === 0 ? <tr><td colSpan="7" className="text-center py-8 text-gray-600">Орлого байхгүй</td></tr> : revenueRecords.map((record) => (
                        <tr key={record.id}>
            ...
            {activeTab === 'expense' && (
              <>
                <table className="data-table min-w-full">
                  ...
                  <tbody>
                    {expenseRecords.length === 0 ? <tr><td colSpan="5" className="text-center py-8 text-gray-600">Зарлага байхгүй</td></tr> : expenseRecords.map((record) => (
                        <tr key={record.id}>
                          <td className="whitespace-nowrap">{formatDate(record.payment_date)}</td>
